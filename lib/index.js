...
/** Persist history to disk (best effort, never throws, writes serialized). */
async function saveHistory(ctx, state) {
  ...
  // Chain onto any in-flight write so rapid clipboard changes never interleave
  // partial history snapshots on disk.
  const pending = (state._writeChain = state._writeChain || Promise.resolve()).then(write)
  state._writeChain = pending.catch(() => {})
  await pending
}
